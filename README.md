...
| Runtime cost | JVM boxing | Zero — static dispatch, inlined |
...
The struct generated by `@FunctionalProtocol` deliberately avoids that convention. The `...Functor` suffix signals the C++/CS meaning: a callable object backed by a closure. It is a **concrete generic type** that conforms directly to the protocol. The compiler can fully specialize and inline it. There is no heap allocation, no dynamic dispatch, no type erasure in the existential sense.
